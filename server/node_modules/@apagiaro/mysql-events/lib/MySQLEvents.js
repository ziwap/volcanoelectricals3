...
class MySQLEvents extends EventEmitter {
  ...
  _queryDatabase(query, queryParams) {
  ...
  _handleEvent(event) {
    ...
    Promise.all(triggers.map(async (trigger) => {
      try {
        const queryResult = trigger.query ? await this._queryDatabase(trigger.query, trigger.queryParams) : null;
        await trigger.onEvent({
          event,
          ...{ data: queryResult } || {},
        });
      } catch (error) {
   ...
   * @param query A query executed before to invoke the onEvent callback
   * @param queryParams The parameters of the query
  ...
  addTrigger({
    ...
    if (!expression) throw new Error('Missing trigger expressions');
